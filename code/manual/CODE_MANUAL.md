...
| 2026-05-06 | **NEW: `Codes/tools/extract_paper_dependencies.py` v1.0 (Paper ↔ Math-note dependency mapper)**. Sweeps every paper TeX under `Docs/papers/{papers,auxiliary,top_impact,epochs}/*.tex` and extracts: (a) `% Canonical archive: ...` curated header tokens (authoritative); (b) every `\cite{Math...}` body reference; (c) every `MathNN` / `MathNN-AddX` mention in body prose. Validates each cited Math note exists in `Docs/math/TECT-Math<NN>*.tex.txt`; missing notes cause exit 1 (override with `--no-fail`). **Outputs**: `Docs/status/PAPER-MATH-DEPENDENCIES.md` (forward map paper→notes + reverse map note→papers, 13 KB) AND `Website/data/papers_math_dependencies.js` (schema `tect-papers-math-deps-v1`, ~36 KB) for browser rendering. **Companion website page**: `Website/papers-deps.html` renders the dependency matrix with live filter, cross-linked to `papers-pdf.html` for download. **Hooked into `Codes/scripts/snapshot.ps1` step 2.5a (before publish-papers)** so the dependency map is always current when PDFs are mirrored. **Coverage on 2026-05-06**: 36 papers cite 179 unique Math notes (117 curated header refs + 348 body refs); 0 missing references after fix. **Reverse-map use-case**: when a Math note is updated, the operator can immediately see which papers depend on it and need re-build. **Maturity**: ACTIVE, production. Cross-references: `Codes/tools/publish_papers.py` (PDF publisher, runs immediately after this), `Website/papers.html` (curated index, links to both deps and pdf pages). |
| 2026-05-06 | **NEW: `Codes/tools/publish_papers.py` v1.0 (Layer 1.5 of publish pipeline)**. Sweeps every `Docs/papers/{papers,auxiliary,top_impact,epochs}/*/Paper-*.pdf` produced by `build-all-papers.ps1` and copies into the public mirror at `Github/assets/papers/<category>/<stem>.pdf` (category-aware subfolders), then regenerates `Website/data/papers_pdf_index.js` (schema `tect-papers-pdf-index-v1`) so the website's new `Website/papers-pdf.html` page can render download links. **Invariants**: (I1) no network; (I2) idempotent (mtime + size checked, only re-copies changed files); (I3) never touches `Github/.git/`; (I4) English-only output. **CLI**: `--check` (dry-run), `--clean` (wipe target first), `-v` (verbose deltas). **Exit codes**: 0 OK; 1 missing PDFs vs .tex sources; 2 I/O error; 3 index write error. **Hooked into `Codes/scripts/snapshot.ps1` between step 2 (generate) and step 3 (verify)**, so the standard snapshot pipeline now: Docs/papers/**/Paper-*.tex → build-all-papers (PDF) → publish_papers (mirror to Github + Website index) → verify-website → curate → push to GitHub. **New website page**: `Website/papers-pdf.html` renders the per-category PDF grid with download buttons; the existing curated `Website/papers.html` was extended with a "Download compiled PDFs" link in its subtitle. **Maturity**: ACTIVE, production. **Total managed**: 36 PDFs across 4 categories (papers=18, auxiliary=2, top_impact=4, epochs=12), ~12 MB total. |
| 2026-05-06 | **NEW (build infrastructure v2.0)**: per-paper builder upgraded + master pipeline added. Five files affected: `Docs/papers/papers/_shared/build-paper.ps1` (PowerShell, v2.0), `Docs/papers/papers/_shared/build-paper.sh` (bash, v2.0), `Codes/scripts/build-all-papers.ps1` (master, v2.0), `Codes/scripts/build-all-papers.sh` (master, v2.0). **Per-paper builder** (`build-paper.{ps1,sh}`): now smart 2/3-pass — pass 1 always, bibtex only if `\bibliography{}` is present AND `.bbl` is stale, pass 2 always, extra passes (capped at `--max-ref-passes`) if `*.log` still reports undefined references. Capture full pdflatex/bibtex stdout to `<base>.build.log`; show only trailing 8 lines on console (or nothing under `--quiet`). New flags: `--clean` (purge aux/bbl/blg/toc/log/out/run.xml/synctex.gz/fls/fdb_latexmk before build), `--no-bibtex` (skip), `--quiet`. Exit codes: 0 OK; 1 pass-1 fail; 2 bibtex fail; 3 pass-2 fail; 4 extra-pass fail; 5 no PDF. **Master pipeline** (`build-all-papers.{ps1,sh}`): walks four canonical categories `{papers, auxiliary, top_impact, epochs}`, dispatches each `Paper-*/Paper-*.tex`, `Auxiliary-*/Auxiliary-*.tex`, `Paper-TI-*/Paper-TI-*.tex`, `Epoch-*/Epoch-*.tex` to the per-paper builder. Color-coded console summary table (Status / ExitCode / PDF KB / ms) with per-category breakdown, JSON build-log emitted to `Runs/build_logs/build-<timestamp>.json` (schema `tect-build-all-papers-v2.0`). Flags: `-Filter <regex>`, `-Categories <csv>`, `-Clean`, `-NoBibtex`, `-Quiet`, `-ListOnly`, `-Parallel <N>` (PowerShell 7+ `ForEach-Object -Parallel`; bash uses `--jobs N` via xargs `-P`). Total target count auto-detects ≈35 papers across the four categories. **Operator workflow**: `cd C:\Dev\TECT2\Contents; .\Codes\scripts\build-all-papers.ps1` builds the entire paper-track in one pass; `.\Codes\scripts\build-all-papers.ps1 -Filter Paper-0[0-3] -Clean` rebuilds Papers 00–03 from scratch; `.\Codes\scripts\build-all-papers.ps1 -Categories epochs -Parallel 4` parallel-builds the 12 Epochs. **Maturity**: ACTIVE, production. **Cross-references**: `Docs/papers/papers/_shared/tect-preamble.tex` (canonical preamble inputted by all 35 papers post the 2026-05-06 audit cycle), `Docs/papers/papers/_shared/tect-references.bib` (canonical shared bibliography). |
