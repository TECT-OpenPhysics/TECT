...
window.TECT_THEORY = {
  ...
  blocks: [
...

    {
      ...
      blocks: [{ type: "html", content:
        "<p>TECT is a candidate Theory of Everything in active development; it is not a finished theory. Four limitations are load-bearing and disclosed transparently here. For each, the corresponding ongoing-closure work is hyperlinked to the relevant Math note or paper.</p>" +
        "<h3>Limitation 1 — the $\\hbar$ derivation route is not yet independent</h3>" +
        "<p>The foundational identification $$\\hbar\\;=\\;\\frac{c^3 a_{\\rm BCC}^2}{16\\pi G}$$ ties Planck constant to the BCC modulation length and the Newton constant. This is a single derivation route, anchored in the elastic-modulus identification of <a href=\"math-notes.html\">Math110-AddI</a>; an independent matter-side route has not yet been written down. <a href=\"math-notes.html\">Math156</a> §3.1 demoted Pillar 10 from PROVED to PROVED CONDITIONAL (weak) on this ground. Closure work in progress: <a href=\"math-notes.html\">Math163</a> (matter-side stress-tensor route), <a href=\"math-notes.html\">Math196</a> (Berry-curvature route), and <a href=\"status.html\">Pillar 10 status row on the Status page</a>.</p>" +
        "<h3>Limitation 2 — Pillar 4 sub-task 3 is in STRONG DRAFT, not unconditional closure</h3>" +
        "<p>The proof that the SM gauge group emerges with the correct three generations is closed at sub-tasks 1 and 2 (PROVED CONDITIONAL via <a href=\"math-notes.html\">Math162</a>+Math167 and <a href=\"math-notes.html\">Math191</a>+<a href=\"math-notes.html\">Math192</a>). Sub-task 3 (residual cohomology integral on the order manifold) remains in the STRONG DRAFT state. Closure work in progress: documented under the active sub-task 3 entry on the <a href=\"status.html\">Status page</a> and the open-question registry behind it. GAP-2 and GAP-3 of the quantum gates auto-resolve once sub-task 3 closes.</p>" +
        ...
        "<h3>Limitation 4 — BCC selection mechanism is open; regime-dependent stability</h3>" +
        "<p>The BCC ordering is the most natural candidate for the energetically selected ordered phase of the Brazovskii universality class, but the selection is not yet a first-principles theorem. Two specific findings, both archived in 2026-05-07, sharpen this limitation:</p>" +
        "<ul>" +
          "<li><strong>Mean-field non-uniqueness</strong> (<a href=\"math-notes.html\">Math348</a>): an explicit one-parameter family of two-latitude prismatic 12-stars $\\{S_h\\}_{h\\in(0,1)}$ achieves the same mean-field resonance functional $\\Lambda(S_h) = 540$ as BCC. Selection of BCC over $S_h$ requires beyond-mean-field input (1-loop Brazovskii corrections, second-shell admixture, or linear-stability order-by-disorder); none has yet been established analytically. The cascading-impact analysis is in <a href=\"math-notes.html\">Math349</a>.</li>" +
          "<li><strong>Regime-dependent stability</strong> (<a href=\"math-notes.html\">Math350</a>): at the deep-regime operating point ($\\mu^2=-0.7$, $N=32$), the Math292 transverse-Hessian acceptance gate (G3) fails decisively: the LOBPCG diagnostic finds five negative eigenvalues with $|\\lambda_{\\min}| = 0.24$ and zero-mode contamination $\\le 4\\times 10^{-14}$, identifying the configuration as a saddle of Morse index $\\ge 5$. The shallow-regime BCC stability anchor (<a href=\"math-notes.html\">Math82-AddD</a> at $\\mu^2 = +5\\times 10^{-3}$) is unaffected, but the deep-regime ground state is currently undetermined. The regime-transition $\\mu^2_*$ at which BCC loses stability lies in the open interval $(-0.7, +5\\times 10^{-3})$.</li>" +
        "</ul>" +
        "<p>Closure work in progress: the new highest-priority mainline target is <strong>M5 — identification of the true deep-regime ground state</strong> (Math349-AddA §2.2), pursued via inspection of the Math350 LOBPCG eigenvectors plus directional Newton--Krylov continuation. Until M5 closes, downstream pillars that use the BCC topology (Pillar 4 gauge group, Pillar 5 Hopf-link, Pillar 8 Hirzebruch index, Pillar 10 $\\hbar$ formula) carry an explicit hypothesis &quot;the operating regime is one in which BCC is at least locally stable&quot;.</p>" +
