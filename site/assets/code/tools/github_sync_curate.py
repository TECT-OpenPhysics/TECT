...
# ---------------------------------------------------------------------
# Section C3. v3 dispatch (Math353-AddE 2026-05-08).
# ---------------------------------------------------------------------
# When mirror.json::active_v3_publish=true, copy_website_tree and
# prune_stale_files dispatch to build_v3_mirror.py logic for the new
# theory-only root structure (note/, paper/, code/, status/, site/).
# v0 Website→Github 1:1 mirror is bypassed.
# ---------------------------------------------------------------------

def _load_mirror_v3_active() -> bool:
    """Read active_v3_publish toggle from mirror.json. Default False."""
    if not MIRROR_CONFIG.exists():
        return False
    try:
        cfg = json.loads(MIRROR_CONFIG.read_text(encoding="utf-8"))
        if int(cfg.get("schema_version", 0)) < 3:
            return False
        return bool(cfg.get("active_v3_publish", False))
    except (OSError, json.JSONDecodeError):
        return False


def _v3_classify():
    """Invoke build_v3_mirror.walk_and_classify(). Returns (cls, cfg)."""
    sys.path.insert(0, str(REPO_ROOT / "Codes" / "scripts"))
    from build_v3_mirror import walk_and_classify, load_config
    cfg = load_config()
    return walk_and_classify(cfg), cfg


def _v3_copy_canonical_subset(stats: CurateStats, *, dry_run: bool) -> None:
    """v3 mode: copy theory-only canonical subset into TARGET (Github/)."""
    sys.path.insert(0, str(REPO_ROOT / "Codes" / "scripts"))
    from build_v3_mirror import walk_and_classify, materialise_to_target, load_config
    cfg = load_config()
    cls = walk_and_classify(cfg)
    if not dry_run:
        materialise_to_target(cls, TARGET, clean=False, preserve_dotgit=True)
    for local, mirror, _size in cls["mirror"]:
        stats.added.append(mirror)
    print(f"       v3 mode: {len(cls['mirror'])} files (skip {len(cls['skip'])})")


def _v3_prune(stats: CurateStats, *, dry_run: bool) -> None:
    """v3 mode: prune any TARGET/ file outside the v3 expected set."""
...
def copy_website_tree(stats: CurateStats, *, dry_run: bool) -> None:
    """Mirror Website/* (or v3 canonical subset if active_v3_publish=true).

    v0 mode: flat copy of Website/ contents into Github/ root.
    v3 mode (Math353-AddE): dispatch to build_v3_mirror.py logic which
    populates Github/ with note/, paper/, code/, status/, site/ structure.
    """
    if _load_mirror_v3_active():
        _v3_copy_canonical_subset(stats, dry_run=dry_run)
        return
...
def prune_stale_files(stats: CurateStats, *, dry_run: bool) -> None:
    """Remove files in Github/ outside the expected mirror set.

    v3 mode dispatches to _v3_prune() (Math353-AddE).
    """
    if _load_mirror_v3_active():
        _v3_prune(stats, dry_run=dry_run)
        return

    """v0 prune: remove files in Github/ that no longer have a source counterpart.
