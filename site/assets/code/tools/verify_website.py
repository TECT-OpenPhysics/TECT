...
# Check 9 (B-ε, Math353-AddC 2026-05-08): rewritten-path resolution.
# Gated on mirror.json::v0_compat_disable_renames toggle. When v0_compat=true
# (default), this is a no-op. When operator flips toggle to false (B-δ
# cutover), the check walks every Markdown link / HTML href / src in
# Github/ mirrored files and asserts the target file exists in the mirror
# tree. Non-resolving paths emit errors so a broken cutover cannot ship.
import json as _json_b_eps
def check_rewritten_paths_resolve() -> None:
    """B-ε path-resolution audit on the publish-side mirror tree."""
    mirror_cfg_path = ROOT / "Codes" / "config" / "mirror.json"
    if not mirror_cfg_path.exists():
        return  # mirror policy absent -> nothing to check
    try:
        cfg = _json_b_eps.loads(mirror_cfg_path.read_text(encoding="utf-8"))
    except (OSError, _json_b_eps.JSONDecodeError):
        return
    if int(cfg.get("schema_version", 0)) < 2:
        return  # pre-v2 schema -> no rewriting layer
    if bool(cfg.get("v0_compat_disable_renames", True)):
        return  # v0 compat mode -> no-op (current default)
    target_dir = ROOT / "Github"
    if not target_dir.exists():
        warnings.append("[mirror-resolve] Github/ tree absent; B-ε skipped")
        return
    import re as _re_b_eps
    md_link_re = _re_b_eps.compile(r"\[[^\]]*\]\(([^)]+)\)")
    html_attr_re = _re_b_eps.compile(r"""(?:href|src|data-src)\s*=\s*['\"]([^'\"]+)['\"]""")
    n_checked = 0
    n_broken = 0
    for ext in ("*.md", "*.html", "*.tex.txt"):
        for fp in target_dir.rglob(ext):
            try:
                txt = fp.read_text(encoding="utf-8")
            except (OSError, UnicodeDecodeError):
                continue
            cites = md_link_re.findall(txt) + html_attr_re.findall(txt)
            for c in cites:
                # Skip URLs, anchors, mailto
                if c.startswith(("http://", "https://", "#", "mailto:", "data:")):
                    continue
                # Strip query/fragment
                c_path = c.split("#")[0].split("?")[0].strip()
                if not c_path:
                    continue
                # Resolve against the file's parent (relative) or target_dir (absolute-ish)
                if c_path.startswith("/"):
                    candidate = target_dir / c_path.lstrip("/")
                else:
                    candidate = (fp.parent / c_path).resolve()
                n_checked += 1
                if not candidate.exists():
                    ...
                    if n_broken <= 20:
                        ...
                        errors.append(
                            f"[mirror-resolve] {rel}: cite '{c}' does not "
                            f"resolve in mirror tree (B-ε path-resolution check)"
                        )
    if n_broken > 20:
        errors.append(
            f"[mirror-resolve] ... (and {n_broken - 20} more broken cites; "
            f"total {n_broken} of {n_checked})"
        )
...
def main() -> int:
    ...
    for js in sorted(DATA.glob("*.js")):
        check_js_syntax(js)
    print("[2/9] Download links …")
    check_download_links()
    print("[3/9] HTML wrappers …")
    check_html_wrappers()
    print("[4/9] Manifest freshness …")
    check_manifest_freshness()
    print("[5/9] Auto-page source counts …")
    check_auto_freshness()
    print("[6/9] Empty TECT_<NAME> blocks …")
    check_empty_tect_objects()
    print("[7/9] Korean text (English-only policy, OUTPUT_LANGUAGE_POLICY.md) …")
    check_korean_text(strict=args.korean_strict)
    print("[8/9] <details>/<summary> pairing (ko-KR auto-translate guard) …")
    check_details_summary()
    print("[9/9] Mirror-tree path resolution (B-ε; gated on v0_compat_disable_renames) …")
    check_rewritten_paths_resolve()
